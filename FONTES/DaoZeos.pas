...
type
  TQueryZeos = class(TInterfacedObject, IQueryParams)
  public
  ...
  TDaoZ = class(TInterfacedObject, IDaoBase)
  private
...
{ TQueryZeos }
procedure TQueryZeos.SetParamFileStream(AProp: TRttiProperty; ACampo: string;
...
begin
  ...
  begin
    ...
    TZQuery(AQry).ParamByName(ACampo).LoadFromStream(stream, ftBlob);
  end
  else
  TZQuery(AQry).ParamByName(ACampo).AsBlob := nil;
end;

procedure TQueryZeos.SetParamStream(AProp: TRttiProperty; ACampo: string;
  ATabela: TTabela; AQry: TObject);
var stream: Tstream;
begin
...
procedure TQueryZeos.SetParamBoolean(AProp: TRttiProperty; ACampo: string;
  ATabela: TTabela; AQry: TObject);
begin
  TZQuery(AQry).ParamByName(ACampo).AsBoolean := AProp.GetValue(ATabela).AsBoolean;
end;

procedure TQueryZeos.SetParamCurrency(AProp: TRttiProperty; ACampo: string;
  ATabela: TTabela; AQry: TObject);
begin
  TZQuery(AQry).ParamByName(ACampo).AsCurrency := AProp.GetValue(ATabela).AsCurrency;
end;

procedure TQueryZeos.SetParamDate(AProp: TRttiProperty;
...
begin
  ...
  if AProp.GetValue(ATabela).AsType<TDate> = 0 then
    ...
    TZQuery(AQry).ParamByName(ACampo).AsDate := AProp.GetValue(ATabela).AsType<TDate>;
end;

procedure TQueryZeos.SetParamDateTime(AProp: TRttiProperty; ACampo: string;
  ATabela: TTabela; AQry: TObject);
begin
 inherited;
...
procedure TQueryZeos.SetParamInteger(AProp: TRttiProperty;
  ACampo: string; ATabela: TTabela; AQry: TObject);
begin
  TZQuery(AQry).ParamByName(ACampo).AsInteger := AProp.GetValue(ATabela).AsInteger;
end;

procedure TQueryZeos.SetParamString(AProp: TRttiProperty;
  ACampo: string; ATabela: TTabela; AQry: TObject);
begin
  TZQuery(AQry).ParamByName(ACampo).AsString := AProp.GetValue(ATabela).AsString;
end;

procedure TQueryZeos.SetParamVariant(AProp: TRttiProperty;
  ACampo: string; ATabela: TTabela; AQry: TObject);
begin
  TZQuery(AQry).ParamByName(ACampo).Value := AProp.GetValue(ATabela).AsVariant;
end;

procedure TQueryZeos.SetCamposFileStream(AProp: TRttiProperty; ACampo: string;
  ATabela: TTabela; AQry: TObject);
begin
  AProp.SetValue(ATabela, TZQuery(AQry).CreateBlobStream(TZQuery(AQry).FieldByName(ACampo),bmWrite));
end;

procedure TQueryZeos.SetCamposBoolean(AProp: TRttiProperty; ACampo: string;
  ATabela: TTabela; AQry: TObject);
begin
  AProp.SetValue(ATabela, TZQuery(AQry).FieldByName(ACampo).AsBoolean);
end;

procedure TQueryZeos.SetCamposCurrency(AProp: TRttiProperty;
  ACampo: string; ATabela: TTabela; AQry: TObject);
begin
  AProp.SetValue(ATabela, TZQuery(AQry).FieldByName(ACampo).AsCurrency);
end;

procedure TQueryZeos.SetCamposDate(AProp: TRttiProperty;
  ACampo: string; ATabela: TTabela; AQry: TObject);
begin
  AProp.SetValue(ATabela, TDate(TZQuery(AQry).FieldByName(ACampo).AsDateTime));
end;

procedure TQueryZeos.SetCamposDateTime(AProp: TRttiProperty; ACampo: string;
  ATabela: TTabela; AQry: TObject);
begin
  AProp.SetValue(ATabela, TZQuery(AQry).FieldByName(ACampo).AsDateTime);
end;

procedure TQueryZeos.SetCamposInteger(AProp: TRttiProperty;
  ACampo: string; ATabela: TTabela; AQry: TObject);
begin
  AProp.SetValue(ATabela, TZQuery(AQry).FieldByName(ACampo).AsInteger);
end;

procedure TQueryZeos.SetCamposStream(AProp: TRttiProperty; ACampo: string;
  ATabela: TTabela; AQry: TObject);
begin
  AProp.SetValue(ATabela, TZQuery(AQry).CreateBlobStream(TZQuery(AQry).FieldByName(ACampo),bmWrite));
end;

procedure TQueryZeos.SetCamposString(AProp: TRttiProperty;
  ACampo: string; ATabela: TTabela; AQry: TObject);
begin
...
constructor TDaoZ.Create(Conexao: TZConnection);
begin
  inherited Create;
  FSql := TPadraoSql.create;
  FParams := TQueryZeos.create;
  FConexao := Conexao;
//  FTransacao := Transacao;

  FQuery := TZQuery.Create(Application);
  FQuery.Connection := FConexao;
end;

destructor TDaoZ.Destroy;
begin
  inherited;
end;
...
begin
  NovaClasse := TGerarClasseZeosLib.Create(TGerarClasseBancoFirebird.Create, Self);
...
procedure TDaoZ.SetDataSet(const Value: TDataSet);
begin
  FDataSet := Value;
end;

procedure TDaoZ.StartTransaction;
begin
  FConexao.StartTransaction;
end;

procedure TDaoZ.Commit;
begin
  FConexao.Commit;
end;

procedure TDaoZ.RollBack;
begin
  FConexao.RollBack;
end;

function TDaoZ.InTransaction: Boolean;
begin
  Result := FConexao.InTransaction;
end;

procedure TDaoZ.Limpar(ATabela: TTabela);
begin
  TAtributos.Get.LimparCampos(ATabela);
end;

function TDaoZ.DbToTabela<T>(ATabela: TTabela; ADataSet: TDataSet)
  : TObjectList<T>;
var
  AuxValue: TValue;
  TipoRtti: TRttiType;
...
function TDaoZ.ConsultaGen<T>(ATabela: TTabela; ACamposWhere: array of string)
  : TObjectList<T>;
var
  Dados: TZQuery;
  Contexto: TRttiContext;
  ...
  PropRtti: TRttiProperty;
begin
  Dados := TZQuery.Create(Application);
  try
...
function TDaoZ.ConsultaAll(ATabela: TTabela; AOrderBy: string): TDataSet;
var
  AQry: TZQuery;
begin
  ...
  AQry := TZQuery.Create(Application);
  try
...
function TDaoZ.SelectAll(ATabela: TTabela; AOrderBy: string): TDataSet;
var AQry : TZQuery;
begin
  AQry := TZQuery.Create(Application);
  try
...
function TDaoZ.ConsultaSql(ASql: string; pFetchAll: Boolean): TDataSet;
var AQry: TZQuery;
begin
  AQry := TZQuery.Create(Application);
...
function TDaoZ.ConsultaSql(ASql: string): TDataSet;
var
  AQry: TZQuery; //TFDQuery;
begin
  AQry := TZQuery.Create(Application);
  with AQry do
  begin
...
function TDaoZ.ConsultaSqlExecute(ASql: string): TDataSet;
var
  AQry: TZQuery;
begin
  AQry := TZQuery.Create(Application);
...
function TDaoZ.ConsultaSql(ASql: string; const ParamList: array of Variant): TDataSet;
var
  AQry: TZQuery;
  i: integer;
begin
...
function TDaoZ.ConsultaSql(ATabela, AWhere: string): TDataSet;
var
  AQry: TZQuery;
begin
  try
...
function TDaoZ.ConsultaTab(ATabela: TTabela; ACampos, ACamposWhere,
...
function TDaoZ.ConsultaTabela(ATabela: TTabela;
  ACamposWhere: array of string): TZQuery;
var
  Dados: TZQuery;
  Contexto: TRttiContext;
  Campo: string;
...
function TDaoZ.ConsultaTab(ATabela: TTabela; ACampos, ACamposWhere: array of string): TDataSet;
var
  Dados: TZQuery;
  Contexto: TRttiContext;
  Campo: string;
...
function TDaoZ.ConsultaTab(ATabela: TTabela;
...
function TDaoZ.GetID(ATabela: TTabela; ACampo: string): Integer;
var
  AQry: TZQuery;
begin
  AQry := TZQuery.Create(Application);
...
function TDaoZ.GetMax(ATabela: TTabela; ACampo: string;
  ACamposChave: array of string): Integer;
var
  AQry: TZQuery;
  Campo: string;
...
function TDaoZ.GetRecordCount(ATabela, AWhere: string): Integer;
var
  AQry: TZQuery;
begin
...
function TDaoZ.GetRecordCount(ATabela: TTabela;
...
function TDaoZ.ExecutaQuery: Integer;
begin
  with FQuery do
  begin
    Prepare();
...
function TDaoZ.Excluir(ATabela: TTabela): Integer;
var
  Campo: string;
  PropRtti: TRttiProperty;
  RttiType: TRttiType;
...
function TDaoZ.Excluir(ATabela: TTabela; AWhere: array of string): Integer;
var
  Campo: string;
  PropRtti: TRttiProperty;
  RttiType: TRttiType;
...
function TDaoZ.Excluir(ATabela: string; AWhereValue: string): Integer;
begin
...
function TDaoZ.ExcluirTodos(ATabela: TTabela): Integer;
begin
  FQuery.close;
  FQuery.sql.Clear;
...
function TDaoZ.Inserir(ATabela: TTabela): Integer;
begin
  Result := Self.Inserir(ATabela, []);
end;

function TDaoZ.Inserir(ATabela: TTabela; ACampos: array of string;
...
function TDaoZ.Salvar(ATabela: TTabela): Integer;
begin
  Result := Self.Salvar(ATabela, []);
end;

function TDaoZ.Salvar(ATabela: TTabela; ACampos: array of string;
  AFlag: TFlagCampos): Integer;
var
  Atributos: IAtributos;
  PropRtti: TRttiProperty;
...
function TDaoZ.Buscar(ATabela: TTabela): Integer;
var
  Dados: TZQuery;
  Campo: string;
  PropRtti: TRttiProperty;
...
function TDaoZ.BuscarID(ATabela: TTabela): TDataSet;
var
  Dados: TZQuery;
  Campo: string;
  PropRtti: TRttiProperty;
